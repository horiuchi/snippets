...
public:
    ...
    long long calc(long long n, int p, int q, int x, int y)
    {
        if (n <= 0)
            return 1;
        map<ll, ll>::const_iterator it = cache.find(n);
        if (it != cache.end()) {
            return it->second;
        }
        ll result = _calc(n, p, q, x, y);
        cache.insert(make_pair(n, result));
        return result;
    }

    long long _calc(long long n, int p, int q, int x, int y)
    {
        ll n1 = n / (ll)p - (ll)x;
        ll n2 = n / (ll)q - (ll)y;
        return calc(n1, p, q, x, y) + calc(n2, p, q, x, y);
    }
